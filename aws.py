...
# a demonstration to pull descriptive information out of AWS - By Malek

# uses env variables for AWS authentication.
# preparation:
# sudo easy_install awscli
# aws configure
## command line test: $ aws ec2 describe-availability-zones
...
print(zonenames)
print("  |instance| name, zone, size, tags, state: ")
def reservations():
    for region in regions:
        ...
        instances = [[x.get('Instances')[0].get('PublicDnsName'), x.get('Instances')[0].get('InstanceType'),\
                      x.get('Instances')[0].get('State').get('Name')] for x in instancelist]
